...
HOSTNAME=$2
if [[ -z "$HOSTNAME" ]]; then
    echo "ERROR: Hostname should be provided as a parameter to the script"
    exit 1
fi
...
sudo bash -c "echo 'HOSTNAME=$HOSTNAME' >> /etc/sysconfig/network"
sudo bash -c "echo 'DHCP_HOSTNAME=$HOSTNAME' >> /etc/sysconfig/network"
